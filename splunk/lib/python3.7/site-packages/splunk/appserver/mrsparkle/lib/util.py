...
def extract_csp_header(required_values, allowed_domains, required_directives):
    csp_values = required_values.copy()
    if allowed_domains != '':
        list_domains = allowed_domains.split(',')
        sanitized_domains = [domain.strip() for domain in list_domains]
        csp_values += sanitized_domains

    csp_values_string = ' '.join(csp_values)
    return '; '.join('%s %s' % (name, csp_values_string) for name in required_directives)
...
class extractCSPHeaders(unittest.TestCase):
    def setUp(self):
        self.required_domains = ["'self'", "data:", "blob:", "*.splunk.com"]
        self.required_directives = ["img-src", "connect-src"]
        self.allowed_domains_1 = "*.google.com, 1.60.2.4"
        self.allowed_domains_2 = "   something invalid, is here   "

    def test_empty_allowed_domains(self):
        actual_csp_header = extract_csp_header(self.required_domains, '', self.required_directives)
        expected_csp_header = "img-src 'self' data: blob: *.splunk.com; connect-src 'self' data: blob: *.splunk.com"
        self.assertEqual(actual_csp_header, expected_csp_header)

    def test_valid_allowed_domains(self):
        actual_csp_header = extract_csp_header(self.required_domains, self.allowed_domains_1, self.required_directives)
        expected_csp_header = "img-src 'self' data: blob: *.splunk.com *.google.com 1.60.2.4; connect-src 'self' data: blob: *.splunk.com *.google.com 1.60.2.4"
        self.assertEqual(actual_csp_header, expected_csp_header)

    def test_invalid_allowed_domains(self):
        actual_csp_header = extract_csp_header(self.required_domains, self.allowed_domains_2, self.required_directives)
        expected_csp_header = "img-src 'self' data: blob: *.splunk.com something invalid is here; connect-src 'self' data: blob: *.splunk.com something invalid is here"
        self.assertEqual(actual_csp_header, expected_csp_header)
