...
class JobsController(BaseController):
    ...
    @staticmethod
    def get_xsl_file_meta_data(filepath):
        meta_data = {
            "file_exists": False,
        ...
        if os.path.exists(filepath) is True:
            meta_data["file_exists"] = True

        # XSLT documents are most often identified with the extensions " .xsl " or " .xslt "
        if file_extension in ['.xsl', '.xslt']:
            meta_data["is_valid_extension"] = True
    ...
    def getJobAsset(self, sid, asset, compat_mode=True, **kwargs):
        ...
        if outputMode == 'json' and output and compat_mode and asset in ['results_preview', 'results', 'events']:
            ...
            if not xsl_file_meta_data.get("file_exists"):
            ...
            elif not xsl_file_meta_data.get("is_valid_extension"):
                cherrypy.response.headers['content-type'] = MIME_HTML
                logger.warn('File xsl="%s" is not available' % xslFilePath)
                output = 'The file you are trying to access does not have a valid extension.'

            elif xslFilePath.startswith(splunkHomePath):
                try:
