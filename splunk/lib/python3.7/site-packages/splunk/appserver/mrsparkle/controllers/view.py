...
REQUIRED_DASHBOARD_STUDIO_CSP_DIRECTIVES = ["img-src", "connect-src"]
...
class ViewController(BaseController):
    ...
    def render(self, app, view_id, action=None, q=None, sid=None, s=None, earliest=None, latest=None, remote_server_list=None, now=None, output='html', setup=None, theme=None, **kw):
        ...
        if is_v2_template and cherrypy.config.get('enforce_dashboards_csp') is True:
            dashboards_csp_allowed_domains = cherrypy.config.get('dashboards_csp_allowed_domains', '').strip()  
            csp_header = util.extract_csp_header(REQUIRED_DASHBOARD_STUDIO_CSP_VALUES, dashboards_csp_allowed_domains, REQUIRED_DASHBOARD_STUDIO_CSP_DIRECTIVES)
            cherrypy.response.headers['Content-Security-Policy-Report-Only'] = csp_header
        ...
        if is_v1p0_template or template == util.getDashboardV1p1TemplateUri() or is_v2_template:
            ...
            dashboardV2Name = util.getDashboardV2Name()
            if is_v2_template and not dashboardV2Name in local_apps:
            ...
            output = self.render_template(template, {'suiTheme': splunk_ui_theme, 'theme': theme, 'app': app, 'page': 'dashboard', 'splunkd': splunkd, 'isDashboardRedirected': is_dashboard_redirected})
            time3 = time.time()
            logger.info('PERF - viewType=fastpath viewTime=%ss templateTime=%ss' % (round(time2-time1, 4), round(time3-time2, 4)))
            if is_v1p0_template:
                data = {
