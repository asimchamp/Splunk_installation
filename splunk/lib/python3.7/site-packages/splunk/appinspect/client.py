...
class AppInspectClient(object):
    ...
    def validate(self, app_path, arg_included_tags="", arg_excluded_tags=""):
        ...
        try:
            ...
            data = { "included_tags": arg_included_tags ,
                     "excluded_tags": arg_excluded_tags }

            logger.info("Submit app to AppInspect with app: {} data: {}".format(app_path, data))
    ...
    def get_validation_status_response(self, id):
        """
        ...
        logger.info("Checking validation status with url: {}".format(url))
        return requests.get(
            url=url,
            headers=headers,
    ...
    def get_validation_status(self, id):
        ...
        try:
            response = self.get_validation_status_response(id)
            if response.status_code != 200:
    ...
    def get_validation_result(self, id):
        """
        Use generated id/sha value to retrieve validation result of the app.
    ...
    def get_validation_report(self, request_id, output_mode="text/html"):
        if not self.authenticated:
            raise AppInspectAuthenticateException()
        try:
            ...
            logger.info("Retrieve AppInspect report for app: {}".format(request_id))
            response = requests.get(url=url, headers=headers)
