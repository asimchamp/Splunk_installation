...
class Model(object):
    ...
    def getRestrictionValues(self, rtype, index=""):
        if rtype not in self.getRestrictionTypes():
            raise ModelException("Unknown restriction type: %s" % rtype)
        if len(index):
            index= 'index="'+index+'"'
        results = splunk.search.searchAll("|metadata type=%ss %s | sort -lastTime | head 50 | table %s" % (rtype,index, rtype), sessionKey=self._sessionKey)
...
def test():
   ...
   print("#1 restriction types: %s" % model.getRestrictionTypes())
   print("#2 restriction type:  %s" % model.getRestrictionType())
   try:
      model.setRestrictionType("blah")
      print("#2a accepted bogus model!")
   except:
      pass
   model.setRestrictionType("source")
   print("#3 restriction type: %s" % model.getRestrictionType())
   assert(model.getRestrictionType() == "source")

   for rtype in model.getRestrictionTypes():
      print("#4.. restriction values for %s: %s" % (rtype, model.getRestrictionValues(rtype)))

   print("#5 restriction types: %s" % model.getRestrictionTypes())
   print("#6 restriction type:  %s" % model.getRestrictionType())

   sampleVal = model.getRestrictionValues(model.getRestrictionType(),index="_internal")[-1]
   model.setRestrictionValue(sampleVal)
   print("#7 restriction %s=%s " % (model.getRestrictionType(), model.getRestrictionValue()))
   ...
   for i in range(0, 5):
      ...
      print("#8.. new extraction id: %s" % model.getFieldExtraction())

   try:
      model.deleteFieldExtraction("nonexistant")
      print("#9 !! should error over non-existant extraction")
   except:
      pass
